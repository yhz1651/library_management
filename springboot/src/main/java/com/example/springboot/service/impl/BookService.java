...
public class BookService implements IBookService {
    ...
    public void save(Book obj) {
        try {
            obj.setCategory(category(obj.getCategories())); // list转换成string
            bookMapper.save(obj);
        } catch (Exception e) {
    ...
    public void update(Book obj) {
        try {
            ...
            obj.setUpdatetime(LocalDate.now()); // 更新时间
    ...
    private String category(List<String> categories) {
        ...
        * */
        ...
        if (CollUtil.isNotEmpty(categories)) {
            categories.forEach(v -> sb.append(v).append(" > ")); // 每一级目录后加上 >
            return sb.substring(0, sb.lastIndexOf(" > ")); // 最后一级目录不加 >
        }
        return sb.toString();
    }
