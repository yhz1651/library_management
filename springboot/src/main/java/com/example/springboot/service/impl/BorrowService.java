...
public class BorrowService implements IBorrowService {
    ...
    public PageInfo<Borrow> page(BaseRequest baseRequest) {
        ...
        for (Borrow borrow : borrows) { // 范围遍历
            LocalDate returnDate = borrow.getReturnDate();
            LocalDate now = LocalDate.now();
            if (now.plusDays(1).isEqual(returnDate)) {  // 当前日期比归还的日期小一天
                borrow.setNote("即将到期");
            } else if (now.isEqual(returnDate)) { // 当前日期和归还日期是一天
                borrow.setNote("已到期");
            } else if (now.isAfter(returnDate)) { // 当前日期在归还日期之后
                borrow.setNote("已过期");
            } else { // 当前日期在归还日期一天以前
                borrow.setNote("正常");
            }
...
//        obj.setId(null);  // 新数据
        ...
        if (book != null) {
            long until = 0;
            if (obj.getRealDate().isBefore(obj.getReturnDate())) { // 按时归还
                until = obj.getRealDate().until(obj.getReturnDate(), ChronoUnit.DAYS); // 还书日期与应还日期之间的时间量
            ...
            user.setAccount(account); // 返还和扣除用户积分
            if (account < 0) {
                // 积分为负，锁定账号
